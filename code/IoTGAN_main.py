...
def gen_samples(gan, sessions):
    ...
    samples[1] = tf.contrib.learn.datasets.mnist.dense_to_one_hot(np.asarray(samples[1]), 10)
    return samples

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--mnist-dir", default='/tmp/mnist-data', help="Directory where mnist downloaded dataset will be stored")
    parser.add_argument("--output-dir", default='IoTGAN', help="Directory where models will be saved")
    ...
    if args.train_digits:
        gan = GAN(task_number)
        #gan.__init__(self, task_number)
        for digit in map(int, args.train_digits.split(',')):
            path = "%s/digit-%d/model" % (args.output_dir, digit)
            path_GAN2 = "%s/digit-%d/GAN2_model" % (args.output_dir, digit)
            path_GAN3 = "%s/digit-%d/GAN3_model" % (args.output_dir, digit)

            if not os.path.exists(os.path.dirname(path)):
                os.makedirs(os.path.dirname(path))
            gan.train_digit(mnist_data, digit, path, path_GAN2, path_GAN3, task_number)
