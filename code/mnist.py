...
class MNIST:
        ...
        def __init__(self):
            ...
            with self.g.as_default():
                ...
                self.accuracy_single = tf.reduce_mean(tf.cast(tf.equal(tf.round(self.y_logit), tf.round(self.y_)), tf.float32))
        ...
        def eval_batch_single(self, batch_x, target_y):
            accuracy = self.sess.run([self.accuracy_single], feed_dict={self.x: batch_x, self.y_: target_y, self.keep_prob: 1.0})
            return accuracy[0]
